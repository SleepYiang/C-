...
//容器的迭代器
//正向迭代器（begin   end）
//迭代器解引用可以修改迭代器指针所替代的值
//const_iterator

//rbegin()返回的是最后一个元素的反向迭代器表示
//rend 返回的是首元素前驱位置的迭代器表示
//vector<int>::reverse——iterator

//函数对象
//通过函数指针调用函数，是没有办法内联的，效率很低，因为有函数调用开销
//通过函数对象调用operator（）可以省略函数的调用开销，比通过函数调用函数（不能通过inlin内联调用）效率高
//因为函数对象是用类生成的，所以可以有太你家相关的成员变量，用来记录函数对象的使用时更多的信息


//泛型算法
//特点一：泛型算法的参数接收的都是迭代器
//特点二：泛型算法的参数话可以接收函数对象（c函数指针）
